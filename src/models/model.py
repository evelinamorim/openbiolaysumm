...
class GATModel(nn.Module):
    def __init__(self, in_size, hid_size, heads):
        ...
        self.gat_layers = nn.ModuleList([
            dgl.nn.GATConv(in_size, hid_size, heads[0], activation=F.elu, allow_zero_in_degree=True),
            dgl.nn.GATConv(hid_size*heads[0], hid_size, heads[1], residual=True, activation=F.elu, allow_zero_in_degree=True),
            dgl.nn.GATConv(hid_size*heads[1], hid_size, heads[2], residual=True, activation=None, allow_zero_in_degree=True)
        ])
...
class GraphEncoder():
    def __init__(self, config):
        self.graphs = {
            "train": pickle.load(open(f"{config['graph_data_path']}/graph_sample_train_with_features.pkl", 'rb')),
            "val": pickle.load(open(f"{config['graph_data_path']}/graph_sample_val_with_features.pkl", 'rb')),
            "test": pickle.load(open(f"{config['graph_data_path']}/graph_sample_test_with_features.pkl", 'rb'))
        }
        self.GM = GATModel(
            config['GAT_dim'],
            config['GAT_embedding_size'],
            heads=[config['GAT_heads'], config['GAT_heads'], config['GAT_heads']]
        )

    def get_graph(self, nodes, edges, article_id):
        node_ids = list(range(len(nodes)))
        id_map = {n: i for i, n in enumerate(nodes)}
        edge_src = []
        edge_dst = []
        for edge in edges:
            if edge[0] in id_map and edge[2] in id_map:
                edge_src.append(id_map[edge[0]])
                edge_dst.append(id_map[edge[2]])
        if len(edge_src) == 0:
            return None
        g = dgl.graph((edge_src, edge_dst), num_nodes=len(nodes))
        return g

    def forward(self, idx, split, device):
        if idx >= len(self.graphs[split]):
            return None
        graph_info = self.graphs[split][idx]
        nodes = graph_info['nodes']
        edges = graph_info['edges']
        node_embeddings = torch.tensor(graph_info['nfeatures'], device=device)
        article_id = graph_info['article_id']
        G = self.get_graph(nodes, edges, article_id)
        if G is None:
            return None
        self.GM = self.GM.to(device)
        G = G.to(device)
        node_embeddings = node_embeddings.to(device)
        graph_embeddings = self.GM(G, node_embeddings)  # [num_nodes, GAT_embedding_size]
        # GoldSack: mean pooling over nodes to get a single vector per graph
        graph_embedding_pooled = graph_embeddings.mean(dim=0)  # [GAT_embedding_size]
        return graph_embedding_pooled
...
class LEDKForConditionalGeneration(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig, is_merge_encoders=False, is_graph_decoder=False, multiplier=0.25):
        ...
        self.lm_head = nn.Linear(config.d_model, self.led.shared.num_embeddings, bias=True)
    ...
    def forward(
        ...
        # Print predicted token IDs for the first sample in the batch
        #with torch.no_grad():
            #pred_ids = torch.argmax(lm_logits, dim=-1)
            #print("Predicted token IDs (model.py):", pred_ids[0][:50].cpu().numpy())
            #print("Input token IDs (model.py):", input_ids[0][:50].cpu().numpy())
