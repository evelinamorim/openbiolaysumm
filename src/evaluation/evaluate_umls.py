...
from src.models.model_umls import LEDKForConditionalGeneration, GraphEncoder
...
config = load_train_config("../../configs/train_config_umls.json")
tokenizer = AutoTokenizer.from_pretrained(config["model_str"])
device = 'cuda' if torch.cuda.is_available() else 'cpu'

# Load best GoldSack model
model = LEDKForConditionalGeneration.from_pretrained(
    os.path.join(config['output_dir'], "Best_epoch"),
    use_cache=False,
    is_merge_encoders=config.get('is_merge_encoders', False),
    is_graph_decoder=config.get('is_graph_decoder', False),
)
...
with torch.no_grad():
    for batch in test_dataloader:
        ...
        max_len = max(g.shape[0] for g in graph_enc_out)
        graph_enc_out = [torch.nn.functional.pad(g, (0,0,0,max_len-g.shape[0]), "constant", 0) for g in graph_enc_out]
        graph_enc_out = torch.stack(graph_enc_out, dim=0).to(torch.float16)
        for key in batch:
...
with open(os.path.join(config['output_dir'], "test_descriptions_umls.json"), "w") as f:
    json.dump({"predictions": all_preds, "references": all_refs}, f, indent=4)
with open(os.path.join(config['output_dir'], "test_results_umlss.json"), "w") as f:
