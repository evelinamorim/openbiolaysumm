...
with open("UMLS_Files/semtype_definitions.txt", "r") as in_file:
    semtype_defs = in_file.readlines()
    semtype_defs = {line.strip().split("|")[0]: line.strip().split("|")[-1] for line in semtype_defs}

with open("UMLS_Files/umls_concept_definitions.txt", "r") as in_file:
...
for split in ['train', 'val', 'test']:
    ...
    with open(f"DSplit/{split}_disc_graphs_complete.jsonl", "r") as in_file:
    ...
    pickle.dump(out_graphs, open(f"DSplit/{split}_graphs_with_features.pkl", 'wb'))
