...
FILTERED_BIGRAMS = BASE_DIR / "filtered_train_bigrams.json"
FILTERED_KEYWORDS = BASE_DIR / "filtered_train_keywords.json"
MISSING_TXT = BASE_DIR / "keywords_missing_from_each_article.txt"
OUTPUT_PATH = BASE_DIR / "missing_keywords_train_bigrams.json"
...
def build_missing(filtered_bigrams: Dict[str, Dict], missing_map: Dict[str, List[str]]) -> List[Dict]:
    ...
    for article_id, missing_keywords in missing_map.items():
        ...
        if not source:
            print(f"Warning: article {article_id} not found in filtered_train_bigrams.json; skipping")
            continue
